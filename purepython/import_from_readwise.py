...
# def get_language_codes(phrases: List[str], openai_model: str) -> List[str] | List[None]:
...
#         futures = {
#             executor.submit(detect, phrases[i], openai_model): i
...
def get_language_codes_multithreaded(
    phrases: List[str], openai_model: str
) -> List[str]:
    # Function to be passed to parallel_map
    def detect_language(phrase: str) -> str:
        result = detect(phrase, openai_model)
...
def make_digest_multithreaded(
    all_data: list, supported_language_codes: List[str], openai_model: str  # =["es"]
) -> List[Tuple[str, str]]:
    phrases = filter_phrases(all_data)
    language_codes = get_language_codes_multithreaded(
        phrases=phrases, openai_model=openai_model
    )
    zipped_list = list(zip(phrases, language_codes))
    filtered_list = [
        (phrase, lang)
        for phrase, lang in zipped_list
...
def make_digest(
    all_data: dict, supported_language_codes: List[str], openai_model: str
) -> List[Tuple[str, str]]:
